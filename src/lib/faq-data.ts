...
/**
 * Home FAQ data — single source of truth (issue #188 item 5).
 *
 * These are the exact Q&A entries rendered by
 * src/components/roycss/faq-section.tsx. The home page (src/app/page.tsx)
 * also emits a schema.org FAQPage JSON-LD built from THIS array via
 * `faqPageJsonLd()`, so the structured data and the rendered content can
 * never drift apart — both come from the same module, by construction.
 */
...
export const FAQ_ENTRIES: FaqEntry[] = [
  ...
  },
  {
    ...
    answer:
      `On the showcase, only ~10KB of CSS loads initially — the rest is injected on demand as you browse. Each effect averages ~1KB, and the npm package ships the full minified stylesheet (~${FULL_CSS_MIN_GZ_KB} KB gzipped for all ${EFFECT_COUNT_FORMATTED} effects) — most projects copy only the effects they use and ship well under 50KB.`,
  },
  {
...
/**
 * Build the schema.org FAQPage structured data from FAQ_ENTRIES —
 * questions/answers are byte-identical to what the FAQ section renders.
 */
