 ...
 *
 * Reads ../dist/effects.json (the published catalog — 1,959 effects with
 * metadata) and ../dist/roycss.css (full CSS source), then writes:
 *
 ...
 * Those two are regenerated by `bun run scripts/build-vscode-effects.ts`
 * (repo root — it imports the TS catalog source directly so cssCode stays
 * in the authored form). This script VERIFIES they are fresh (same effect
 * count) and fails the build if they are stale, so the shipped extension
 * can never mix a 1,569-era hover dataset with a 1,959-era completion set.
 *
...
if (effects.length < 1959) {
  throw new Error(
    `Expected ≥1959 effects in dist/effects.json, got ${effects.length}. ` +
...
// Sanity assertions
if (classData.effects.length < 1959) {
  throw new Error(`Expected ≥1959 effects, got ${classData.effects.length}`);
}
