// Package deploy is the Go stub for the "deploy" RoyCSS platform module.
//
// Deploy routes — /api/v1/deploy
//
// Status: STUB — mirrors backend-node/src/modules/deploy.
...
// RegisterRoutes mounts the deploy module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/deploy", notImplemented)
	mux.HandleFunc("/api/v1/deploy/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"deploy"}}`))
}
