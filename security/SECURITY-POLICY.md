...
- **Scope:** All code shipped from this repository — the `roycss`
  npm package, the Next.js marketing site (roycss.com), the VS Code
  extension, the CLI, and the MCP server.
- **Related:** `docs/SECURITY-SLA.md` (the binding timeline policy),
  `security/CSP.md`, `security/CHECKLIST.md` (pre-release checklist),
  `security/README.md` (the audit harness)
...
**Preferred:** Email **security@roycss.dev** with:

> **Provisioning note (honest):** `security@roycss.dev` is the canonical
> address by policy (it unifies what this document previously called
> `security@roycss.com`), but the mailbox is **not yet provisioned** —
> creating it is an owner action. Until it is live, the **reliable**
> private channel is GitHub Security Advisories (below). This note will be
> removed when the mailbox exists.
...
<https://github.com/Roy-Wanyoike/Roycss/security/advisories/new>.
...
(No browser/Inspector extension is shipped yet — PF-038 has not been
started. It joins this scope only when it exists.)
...
- See the `overrides` table in the repository root `package.json`
  (every entry carries a documented rationale; verified by item 1.5 of
...
| Purpose | Contact |
|---|---|
| Security vulnerabilities | **security@roycss.dev** (provisioning pending — see §1.1; PGP key: TBD) |
| General security questions | GitHub Discussions: <https://github.com/Roy-Wanyoike/Roycss/discussions> |
| Security advisories (private) | <https://github.com/Roy-Wanyoike/Roycss/security/advisories/new> |
...
- **Static-safe strict CSP** — the production policy served by
  `src/proxy.ts` is identical for prerendered and dynamic pages:
  `script-src 'self' 'unsafe-inline'`, `frame-ancestors 'none'`,
  `connect-src 'self'`, `object-src 'none'`. Per-request nonces and
  `strict-dynamic` are **banned** here (postmortem #54: they silently
  broke every script on statically prerendered pages). Dev-only
  relaxations (`unsafe-eval`, HMR WebSockets) live in `next.config.ts`.
  See `security/CSP.md`.
...
- **Contact form:** 4-layer input validation, Prisma parameterized
  ...
  and spam-protection requirements are tracked as release-checklist
  items in `security/CHECKLIST.md` §7.
...
- **`bun audit`** runs as a CI gate. Any high/critical advisory fails
  the build.
- **SBOM** (CycloneDX 1.4, regenerated by `security/sbom.ts` into the
  gitignored `security/results/` directory) is generated on every release.

### 6.3 VS Code extension, CLI, MCP server

- All Tier A artifacts ship with **zero runtime dependencies**.
- The VS Code extension's webview CSP is `default-src 'none'` + nonce'd
  scripts (see `vscode-extension/src/extension.ts`).
...
- **Swag** (RoyCSS stickers + t-shirt) for any accepted High/Critical
  report.
- **Hall of fame** listing on
  [`/security/hall-of-fame`](https://roycss.com/security/hall-of-fame)
  (live page; currently no public disclosures).
...
| 2026-09-12 | v1.1 — security mailbox unified to `security@roycss.dev` (was `security@roycss.com`; provisioning pending note added); committed SBOM/dependency-audit snapshots removed (outputs are regenerated by the `security/` harnesses and gitignored); sandbox-path leaks removed from scope/commands. |
| 2026-09-22 | v1.2 — Chrome Inspector extension removed from scope (PF-038 not started); dead `docs/adr/` + `docs/checklists/` references replaced with existing files; §6.1 CSP bullet reconciled with the static-safe policy in `src/proxy.ts` (nonces banned per postmortem #54); contact-form audit retired into `security/CHECKLIST.md` §7. |
| 2026-09-23 | v1.3 — §7 hall-of-fame TODO removed: the `/security/hall-of-fame` page is live (issue #218), listing the honest empty state until the first accepted report. |

---

## 9. References

- `security/CSP.md` — CSP deep-dive (static-safe policy, #54 postmortem)
- `security/CHECKLIST.md` — pre-release checklist
- `security/README.md` — the five security harness scripts (outputs
