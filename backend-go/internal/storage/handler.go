// Package storage is the Go stub for the "storage" RoyCSS platform module.
//
// Storage routes — /api/v1/storage
//
// Status: STUB — mirrors backend-node/src/modules/storage.
...
// RegisterRoutes mounts the storage module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/storage", notImplemented)
	mux.HandleFunc("/api/v1/storage/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"storage"}}`))
}
