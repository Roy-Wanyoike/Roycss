// Package startingstyle is the Go stub for the "starting-style" RoyCSS platform module.
//
// Starting-style routes — /api/v1/starting-style
//
// Status: STUB — mirrors backend-node/src/modules/starting-style.
...
package startingstyle

import (
	"net/http"
)

// RegisterRoutes mounts the starting-style module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/starting-style", notImplemented)
	mux.HandleFunc("/api/v1/starting-style/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"starting-style"}}`))
}
