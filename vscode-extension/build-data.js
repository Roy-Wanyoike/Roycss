 ...
 *
 * Reads ../dist/effects.json (the published catalog — 1,983 effects with
 * metadata as of the #212 refresh; re-baseline the ≥ floors below when the
 * catalog grows) and ../dist/roycss.css (full CSS source), then writes:
 *
 ...
 *
 * Related generated files (NOT written by this script):
 *   - data/css-data.json   — full cssCode map keyed by effect id (lazy-loaded
 *                            by the TS providers' hover/playground).
 *   - src/effects-data.ts  — compact embedded metadata for the TS providers.
 * Those two are regenerated by `bun run build:vscode` (repo root —
 * one-shot regen for ALL FOUR artifacts; it runs
 * scripts/build-vscode-effects.ts, which imports the TS catalog source
 * directly so cssCode stays in the authored form, then this script).
 * This script VERIFIES they are fresh (same effect
 * count) and fails the build if they are stale, so the shipped extension
 * can never mix a stale hover dataset with a fresh completion set.
 *
...
if (effects.length < 1983) {
  throw new Error(
    `Expected ≥1983 effects in dist/effects.json, got ${effects.length}. ` +
...
// ────────────────────────────────────────────────────────────────────────
// 4b. Cross-check the sibling generated files (data/css-data.json +
//     src/effects-data.ts) so a stale hover dataset cannot ship alongside
//     a fresh completion dataset.
// ────────────────────────────────────────────────────────────────────────

const CSS_DATA_PATH = path.join(__dirname, "data", "css-data.json");
const EFFECTS_DATA_TS_PATH = path.join(__dirname, "src", "effects-data.ts");
const expectedCount = classData.effects.length;

if (!fs.existsSync(CSS_DATA_PATH)) {
  throw new Error(
    "data/css-data.json is missing — run " +
      "`bun run scripts/build-vscode-effects.ts` in the repo root, then retry.",
  );
}
const cssMapKeyCount = Object.keys(
  JSON.parse(fs.readFileSync(CSS_DATA_PATH, "utf8")),
).length;
if (cssMapKeyCount !== expectedCount) {
  throw new Error(
    `data/css-data.json is stale: ${cssMapKeyCount} entries vs ` +
      `${expectedCount} effects in dist/effects.json. Run ` +
      "`bun run scripts/build-vscode-effects.ts` in the repo root, then retry.",
  );
}
const effectsDataTs = fs.readFileSync(EFFECTS_DATA_TS_PATH, "utf8");
const headerCount = /Effects:\s*(\d+)/.exec(effectsDataTs);
if (!headerCount || Number(headerCount[1]) !== expectedCount) {
  throw new Error(
    `src/effects-data.ts is stale: header says ${headerCount ? headerCount[1] : "?"} ` +
      `effects vs ${expectedCount}. Run ` +
      "`bun run scripts/build-vscode-effects.ts` in the repo root, then retry.",
  );
}
console.log(
  `[build-data]   ✓ data/css-data.json (${cssMapKeyCount} entries) + ` +
    `src/effects-data.ts in sync with the ${expectedCount}-effect catalog`,
);
...
// Sanity assertions (floor re-baselined with the catalog — 1,983 at #212)
if (classData.effects.length < 1983) {
  throw new Error(`Expected ≥1983 effects, got ${classData.effects.length}`);
}
