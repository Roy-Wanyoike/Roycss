// Package registry is the Go stub for the "registry" RoyCSS platform module.
//
// Registry routes — /api/v1/registry
//
// Status: STUB — mirrors backend-node/src/modules/registry.
...
// RegisterRoutes mounts the registry module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/registry", notImplemented)
	mux.HandleFunc("/api/v1/registry/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"registry"}}`))
}
