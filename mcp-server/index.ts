...
const FALLBACK_PATTERNS_DATA: PatternsFile = {
  ...
  patterns: [
    ...
    { id: "pattern-success-state", name: "Success State", category: "states", description: "A success state with confetti and confirmation", whenToUse: "When a user completes a significant action. Use confetti for delight.", effectIds: ["anim-confetti-burst-b20"], tags: ["success", "state", "confetti", "celebration"], html: '<div style="display:flex;flex-direction:column;align-items:center;gap:1rem;padding:2rem;"><div class="roycss-anim-confetti-burst-b20"><span></span><span></span><span></span><span></span><span></span><span></span></div><h3>Success!</h3><p>Your changes have been saved.</p></div>' },
    { id: "pattern-offline-state", name: "Offline State", category: "states", description: "An offline indicator with pulsing dot", whenToUse: "When the app detects no internet. Show a banner, not a full-page block.", effectIds: ["anim-notification-dot-b20"], tags: ["offline", "state", "connectivity", "banner"], html: '<div style="display:flex;align-items:center;gap:0.75rem;padding:0.75rem 1.5rem;"><div class="roycss-anim-notification-dot-b20"></div><p>You\'re offline. Changes will sync when you reconnect.</p></div>' },
    { id: "pattern-skeleton-state", name: "Skeleton Loading", category: "feedback", description: "Skeleton placeholder that mimics content layout", whenToUse: "When loading content with a known layout. Skeletons feel faster than spinners.", effectIds: ["loader-skeleton-card-b20", "loader-skeleton-text-b20"], tags: ["skeleton", "loading", "placeholder", "layout"], html: '<div style="display:flex;flex-direction:column;gap:1rem;padding:1rem;"><div class="roycss-loader-skeleton-card-b20"></div><div class="roycss-loader-skeleton-text-b20"></div></div>' },
    { id: "pattern-progressive-disclosure", name: "Progressive Disclosure", category: "feedback", description: "Content that reveals more on interaction", whenToUse: "When you have a lot of content but don't want to overwhelm.", effectIds: ["micro-accordion-expand-b20"], tags: ["progressive", "disclosure", "accordion", "expand"], html: '<div style="display:flex;flex-direction:column;gap:0.5rem;inline-size:200px;"><div class="roycss-micro-accordion-expand-b20">Click to expand...</div></div>' },
    { id: "pattern-toast-feedback", name: "Toast Feedback", category: "feedback", description: "A slide-in toast for non-blocking feedback", whenToUse: "When you need to confirm an action without blocking. Auto-dismiss after 3-5 seconds.", effectIds: ["micro-toast-slide-b20"], tags: ["toast", "feedback", "notification", "slide"], html: '<div class="roycss-micro-toast-slide-b20">✓ Saved successfully</div>' },
...
const A11Y_GUIDANCE = {
  ...
  effectSpecific: {
    ...
    "loader-skeleton-card-b20": "Mark with role=\"status\" + aria-busy=\"true\" on the parent. Skeletons are decorative.",
    "anim-confetti-burst-b20": "Hide from screen readers (aria-hidden=\"true\"). Confetti is purely decorative.",
    ...
    "micro-toast-slide-b20": "Use role=\"status\" (non-urgent) or role=\"alert\" (urgent). Auto-dismiss after 3-5s but keep focusable if it has an action.",
    "nav-stepper-b20": "Wrap in <ol>. Use aria-current=\"step\" on the current step. Use aria-disabled=\"true\" on unreachable steps.",
    "micro-accordion-expand-b20": "Use <button aria-expanded>. The pattern HTML in patterns.json is a visual example; the production markup must use proper button semantics.",
  ...
  tools: [
    ...
    },
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          effect_id: { type: "string", description: "Optional: effect ID to get effect-specific a11y notes (e.g., 'loader-spinner', 'micro-toast-slide-b20', 'nav-stepper-b20')." },
        },
      },
    },
    {
  ...
  try {
    if (promptName === "design-a-landing-page") {
      ...
      const vibeToEffect: Record<string, string> = {
        ...
        playful: "anim-confetti-burst-b20",
...
3. Call get_accessibility_considerations({ effect_id: "${waitDuration === "long" ? "loader-skeleton-card-b20" : "loader-spinner"}" }) — loaders must be marked aria-hidden or role=\"status\" depending on context.
