// Package relativecolor is the Go stub for the "relative-color" RoyCSS platform module.
//
// Relative-color routes — /api/v1/relative-color
//
// Status: STUB — mirrors backend-node/src/modules/relative-color.
...
package relativecolor

import (
	"net/http"
)

// RegisterRoutes mounts the relative-color module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/relative-color", notImplemented)
	mux.HandleFunc("/api/v1/relative-color/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"relative-color"}}`))
}
