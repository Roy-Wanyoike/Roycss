// Package fallback is the Go stub for the "fallback" RoyCSS platform module.
//
// Fallback routes — /api/v1/fallback
//
// Status: STUB — mirrors backend-node/src/modules/fallback.
...
// RegisterRoutes mounts the fallback module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/fallback", notImplemented)
	mux.HandleFunc("/api/v1/fallback/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"fallback"}}`))
}
