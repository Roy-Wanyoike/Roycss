...
/**
 * Embedded mode — what `/api/v1/*` and `/api/health` do when the site
 * runs WITHOUT a configured backend (issue #245; behavior per the #83
 * mode-aware gateway, black-box verified live).
 */
function embeddedModeSection(): string {
  return `
## Embedded mode
...
function generate(): string {
  ...
  out.push("- [Embedded mode](#embedded-mode) — standalone behavior: catalog reads, 503 EMBEDDED_MODE_UNSUPPORTED, /api/health apiMode");
  out.push("");
  ...
  out.push("| 503 | `EMBEDDED_MODE_UNSUPPORTED` | Frontend-only (embedded API mode): a write, auth or unknown-module request the read-only catalog cannot serve — see [Embedded mode](#embedded-mode) |");
  out.push("");
  out.push("### Auth");
  out.push("");
