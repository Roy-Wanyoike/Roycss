// Package sync is the Go stub for the "sync" RoyCSS platform module.
//
// Sync routes — /api/v1/sync
//
// Status: STUB — mirrors backend-node/src/modules/sync.
...
// RegisterRoutes mounts the sync module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/sync", notImplemented)
	mux.HandleFunc("/api/v1/sync/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"sync"}}`))
}
