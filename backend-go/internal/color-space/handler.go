// Package colorspace is the Go stub for the "color-space" RoyCSS platform module.
//
// Color-space routes — /api/v1/color-space
//
// Status: STUB — mirrors backend-node/src/modules/color-space.
...
// RegisterRoutes mounts the color-space module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/color-space", notImplemented)
	mux.HandleFunc("/api/v1/color-space/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"color-space"}}`))
}
