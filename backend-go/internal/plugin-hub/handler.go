// Package pluginhub is the Go stub for the "plugin-hub" RoyCSS platform module.
//
// Plugin Hub routes — /api/v1/plugins
//
// Status: STUB — mirrors backend-node/src/modules/plugin-hub.
...
// RegisterRoutes mounts the plugin-hub module's routes on the given mux.
// All routes return 501 until the Go implementation is completed.
func RegisterRoutes(mux *http.ServeMux) {
	mux.HandleFunc("/api/v1/plugin-hub", notImplemented)
	mux.HandleFunc("/api/v1/plugin-hub/", notImplemented)
}
...
func notImplemented(w http.ResponseWriter, _ *http.Request) {
	...
	_, _ = w.Write([]byte(`{"error":{"code":"NOT_IMPLEMENTED","message":"Go stub — use backend-node for this module","module":"plugin-hub"}}`))
}
