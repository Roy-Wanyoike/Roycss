...
- **Data refresh to the 1,959-effect catalog** (issue #132). The extension
  ...
  - `class-data.json` — 1,569 → 1,959 effects (regenerated by
    `node build-data.js` from `../dist/effects.json` + `../dist/roycss.css`).
  - `snippets.json` — 1,569 → 1,959 snippets.
  - `data/css-data.json` — 1,569 → 1,959 entries (regenerated by
    `bun run scripts/build-vscode-effects.ts`; authored `cssCode`).
  - `src/effects-data.ts` — 1,569 → 1,959 embedded metadata entries.
- `build-data.js` now **cross-checks** `data/css-data.json` and
  `src/effects-data.ts` against `dist/effects.json` and fails the build if
  they are stale, so a hover dataset can never ship out of sync with the
  completion dataset again.
- `roycss.maxCompletionItems` default/maximum raised 1569 → 1959 (settings
  now expose the full catalog); effect-count claims in `package.json`,
  `extension.js` (load guard ≥1959, completion cap 1959), and `README.md`
  updated to 1,959.
