...
- **Related:** [`security/SECURITY-POLICY.md`](../security/SECURITY-POLICY.md)
  ...
  [`security/README.md`](../security/README.md) (the five security harness
  scripts — outputs regenerated on demand, gitignored) ·
...
PGP-encrypted — key policy in §4; **provisioning note:** the mailbox is not
yet provisioned — an owner action — so GitHub Security Advisories, §2.2,
are the reliable private channel until it exists). We acknowledge within 24
hours, assess within 72 hours, and **ship a fix for Critical vulnerabilities
within 72 hours of confirmation**. Disclosure is coordinated — 90 days by
default — through GitHub Security Advisories, with CVEs requested from MITRE
via GitHub. Critical fixes are backported to the active LTS line per
...
> **Provisioning status (honest):** the mailbox itself is **not yet
> provisioned** (creating it is an owner action — the repo docs previously
> disagreed between `security@roycss.com` and `security@roycss.dev`; `.dev` is
> canonical from 2026-09-12). Until it exists, GitHub Security Advisories
> (§2.2) are the monitored channel, and this note should be removed the day
> the mailbox goes live.
...
- **SBOM** (CycloneDX 1.4, regenerated by `security/sbom.ts` into the
  gitignored `security/results/` directory) is generated per release;
  dependency advisories are audited by the `bun audit` release gate in
  [`security/CHECKLIST.md`](../security/CHECKLIST.md) §1 (re-run after every
  dependency change).
...
accepted risk in the supply-chain ADR, within the same severity clocks.
...
| 2026-09-12 | References updated for the audit-report removal: the committed SBOM/dependency-audit snapshots are gone (outputs are regenerated by the `security/` harnesses and gitignored); mailbox `security@roycss.dev` confirmed canonical with an explicit provisioning-pending note. |
